(function () {
  ...
  function toggleRoleFields() {
    ...
    if (isLegal) {
      ...
      document
        .getElementById("signup-companyName")
  ...
  function showFieldErrors(errors) {
    ...
    Object.keys(errors).forEach(function (field) {
      ...
      if (el)
        el.textContent = Array.isArray(errors[field])
          ? errors[field][0]
          : errors[field];
  ...
  function buildSignupBody() {
    ...
    const body = {
      ...
      postalCode: document
        .getElementById("signup-postalCode")
        .value.toString()
        .trim(),
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(function () {
          ...
          } else if (data.data && !data.data.role && data.data.fullName) {
            window.location.href = "dispecer.html";
          } else {
      ...
      if (
        data.statusCode === 401 &&
        msg.toLowerCase().indexOf("email") !== -1
      ) {
  ...
  formSignup.addEventListener("submit", async function (e) {
    ...
    if (password !== passwordConfirm) {
      document.getElementById("signup-passwordConfirm-error").textContent =
        "Lozinke se ne poklapaju.";
      document
        .getElementById("signup-passwordConfirm")
    ...
    try {
      ...
      if (res.ok) {
        showMessage(
          "form-signup",
          "Uspešno ste se registrovali. Možete se prijaviti.",
          "success",
        );
        formSignup.reset();
        toggleRoleFields();
